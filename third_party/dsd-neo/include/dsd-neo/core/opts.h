...
struct dsd_opts {
    ...
    FILE* p25_sm_log_f;                   // optional P25 state-machine diagnostic sink
    ...
    int p25_sm_log_open_error_reported;  // guard repeated open error spam
    int p25_sm_log_write_error_reported; // guard repeated write error spam
    ...
    /* SiteSim: when 1, interpret received TSBKs as ISP (inbound from mobile)
     * rather than OSP (outbound from site). Used by site simulators listening
     * on the site input channel (where mobiles transmit). */
    int p25_isp_mode;
